...
`src/lib/Mongbat.lua` first, then update the consuming mod. **Never** weaken
...
- `src/lib/Mongbat.lua` — search for an existing wrapper that resembles what
...
2. **Add the wrapper** in the matching section of `src/lib/Mongbat.lua`.
...
5. **Verify.** Run `get_errors` on:
   ...
   - `src/lib/Mongbat.lua` → expect no NEW errors (existing engine-global
