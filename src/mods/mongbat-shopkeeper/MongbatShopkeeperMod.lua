...
local function OnInitialize()
    ...
    -- -----------------------------------------------------------------------
    -- Update a single row's displayed fields in-place
    -- Called from per-row event handlers to re-render without a full refresh.
    -- -----------------------------------------------------------------------
    --- @param rowName string  Engine window name of the row
    --- @param item table      The items[] entry for this row
    --- @param qty integer     The quantity to display (availQty or cartQty)
    local function updateRowDisplay(rowName, item, qty)
    ...
    -- -----------------------------------------------------------------------
    -- Update the total/gold label from current cart state
    -- -----------------------------------------------------------------------
    ...
    local function loadBuyItems()
        ...
        Utils.Array.ForEach(data.ContainedItems, function(slot)
            ...
            local itemName = Data.ItemProperties(slot.objectId):getName()
    ...
    local function createShopWindow(mId, isSell)
        ...
        -- In buy mode, OnUpdateContainerWindow belongs here: it is the child that
        -- owns the available items list and triggers a full reload when the
        -- container data changes (e.g. vendor restock).
        local availListModel = {
            ItemHeight = ROW_H,
            OnInitialize = function(self)
        ...
        if not isSelling then
            availListModel.OnUpdateContainerWindow = function(_, instanceId, _)
        ...
        availList = Components.ScrollWindow(availListModel)
        ...
        -- OnUpdatePlayerStatus belongs here: it is the only component that
        -- displays gold data, so it should receive gold updates directly.
    ...
    shopkeeperDefault:getDefault().Initialize = function()
        ...
        -- For buy mode: derive sellContainerId from ObjectInfo
        if not isSell then
